...
function isAlignmentRowArray(x: unknown): x is AlignmentRow[] {
...
export async function fetchAlignmentRange(symbol: string, minutes = 120): Promise<AlignmentResponse> {
  ...
  const points: AlignmentPoint[] = isAlignmentRowArray(rowsUnknown)
    ? rowsUnknown.map((r) => ({
...
export type StrategyConfig = { symbol: string; z_threshold: number; percentile_min: number };

export async function fetchStrategyConfig(symbol: string): Promise<StrategyConfig> {
  const res = await fetch(
    `${API_BASE}/api/strategy/config?symbol=${encodeURIComponent(symbol)}`,
    { cache: "no-store" }
  );
  await mustOk(res);
  return res.json();
...
export async function fetchPriceRange(symbol: string, minutes = 120): Promise<PricePoint[]> {
  const res = await fetch(
    `${API_BASE}/api/price/range?symbol=${encodeURIComponent(symbol)}&minutes=${minutes}`,
    { cache: "no-store" }
  );
  await mustOk(res);
  return res.json();
